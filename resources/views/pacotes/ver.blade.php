...
@section('title', 'Visualizar Pacote')

@section('content_header')
    <h1>
        Detalhes do Pacote #{{ $pacote->id ?? request('id') }}
        <div class="float-right">
            @if(Auth::user()->role == 'admin' || Auth::user()->role == 'protocolo')
                <button id="receber-pacote" class="btn btn-primary btn-sm mr-2">
                    <i class="fas fa-plus"></i> Receber Pacote
                </button>
            @endif
            <a href="{{ route('pacotes.index') }}" class="btn btn-sm btn-secondary">
...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Pacote #{{ $pacote->id ?? request('id') }}</li>
    </ol>
        ...
        <div class="col-md-3">
            <div class="card">
                ...
                <div class="card-body">
                    <dl>
                        <dt>Nº do Pacote</dt>
                        <dd>{{ $pacote->id ?? 'Não definido' }}</dd>
                        
                        <dt>OCS/PSA</dt>
                        <dd>{{ optional($pacote->ocsPsa)->nome ?? 'Não definido' }}</dd>
                        
                        <dt>Data da Entrada no Protocolo</dt>
                        <dd>{{ optional($pacote->data_entrada)->format('d/m/Y') ?? 'Não definido' }}</dd>
                        
                        <dt>Número da Fatura</dt>
                        <dd>{{ $pacote->numero_fatura ?? 'Não definido' }}</dd>
                        
                        <dt>Tipo</dt>
                        <dd>{{ optional($pacote->tipoPacote)->nome ?? 'Não definido' }}</dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <dl>
                        ...
                        <dd>R$ {{ number_format($pacote->valor_fatura, 2, ',', '.') }}</dd>
                        
                        <dt>Glosa</dt>
                        <dd>R$ {{ number_format($pacote->valor_glosa, 2, ',', '.') }}</dd>
                        
                        <dt>Valor Pós Lisura</dt>
                        <dd>R$ {{ number_format($pacote->valor_pos_lisura, 2, ',', '.') }}</dd>
                        
                        <dt>Valor Implantado</dt>
                        <dd>R$ {{ number_format($pacote->valor_pago, 2, ',', '.') }}</dd>
                        
                        <dt>Valor Pendente</dt>
                        <dd>R$ {{ number_format($pacote->valor_pendente, 2, ',', '.') }}</dd>
                    </dl>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3">
            <div class="card">
                ...
                <div class="card-body">
                    <dl>
                        <dt>Tipo de Conta</dt>
                        <dd>{{ optional($pacote->tipoConta)->nome ?? 'Não informado' }}</dd>
                        
                        <dt>Motivo da Glosa</dt>
                        <dd>{{ $pacote->motivo_glosa ?? 'Não informado' }}</dd>
                        
                        <dt>Recurso de Glosa</dt>
                        <dd>R$ {{ number_format($pacote->valor_recurso_glosa ?? 0, 2, ',', '.') }}</dd>
                        
                        <dt>Valor Recursado</dt>
                        <dd>R$ {{ number_format($pacote->valor_recursado ?? 0, 2, ',', '.') }}</dd>
                        
                        <dt>Valor Deferido</dt>
                        <dd>R$ {{ number_format($pacote->valor_deferido ?? 0, 2, ',', '.') }}</dd>
                    </dl>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3">
            <div class="card">
                ...
                <div class="card-body">
                    <dl>
                        <dt>Estado Geral</dt>
                        <dd>{{ $pacote->estado_geral }}</dd>
                        
                        <dt>Estado da Glosa</dt>
                        <dd>{{ $pacote->estado_glosa }}</dd>
                        
                        <dt>Localização Atual</dt>
                        <dd>{{ ucfirst($pacote->localizacao_atual) }}</dd>
                        
                        <dt>Localização Anterior</dt>
                        <dd>{{ ucfirst($pacote->localizacao_anterior) }}</dd>
                        
                        <dt>Última Ação</dt>
                        <dd>{{ $pacote->ultima_acao }}</dd>
                    </dl>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                    <div class="progress-stages">
                        <div class="row text-center">
                            ...
                            <!-- Protocolo -->
                            <div class="col">
                                <div class="stage {{ $posicaoAtual >= 0 ? 'active' : '' }}">
                            ...
                            <!-- Lisura -->
                            <div class="col">
                                <div class="stage {{ $posicaoAtual >= 1 ? 'active' : '' }}">
                            ...
                            <!-- SIRE -->
                            <div class="col">
                                <div class="stage {{ $posicaoAtual >= 2 ? 'active' : '' }}">
                            ...
                            <!-- Glosa -->
                            <div class="col">
                                <div class="stage {{ $posicaoAtual >= 3 ? 'active' : '' }}">
                            ...
                            <!-- Arquivo -->
                            <div class="col">
                                <div class="stage {{ $posicaoAtual >= 4 ? 'active' : '' }}">
        ...
        <div class="col-md-3">
            <div class="card">
                ...
                <div class="card-body text-center">
                    ...
                    @endphp
                    <input type="text" class="knob" value="{{ $percentualPago }}" data-thickness="0.2" data-width="90" 
                           data-height="90" data-fgColor="#28a745" data-readonly="true">
                    
                    <div class="mt-3">
                        <span>
                            R$ {{ number_format($pacote->valor_pago, 2, ',', '.') }} 
                            de R$ {{ number_format($valorTotal, 2, ',', '.') }}
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="row mb-4">
        <div class="col-md-12">
            {{-- Botão Mover Pacote - visível para admin ou se o papel do usuário corresponder à localização atual do pacote
                 EXCETO para equipe glosa e arquivo que não têm acesso a este botão --}}
            ...
            {{-- Botão Editar Pacote - só visível para admin, protocolo e lisura --}}
            @if(Auth::user()->role == 'admin' || 
               ((Auth::user()->role == 'protocolo' || Auth::user()->role == 'lisura') && 
                (Auth::user()->role == $pacote->localizacao_atual)))
                <a href="{{ route('pacotes.edit', $pacote->id) }}" class="btn btn-primary">
                    <i class="fas fa-edit"></i> Editar Pacote
                </a>
            @endif
            
            {{-- Botão Movimentações - visível para todos --}}
            <a href="{{ route('pacotes.movimentacoes', $pacote->id) }}" class="btn btn-info">
                <i class="fas fa-history"></i> Movimentações
            </a>
            
            {{-- Botão Prazos e Notificações - visível para todos --}}
            <a href="{{ route('pacotes.prazos', $pacote->id) }}" class="btn btn-warning">
                <i class="fas fa-calendar-alt"></i> Prazos e Notificações
            </a>

            {{-- Botão Recebimento de Recurso de Glosa - visível apenas para admin ou protocolo
                 e somente quando o estado da glosa FOR "Aguardando Recurso de Glosa" --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'protocolo') && 
            ...
            {{-- Botão Notificação de Existência de Glosa - visível apenas para admin ou equipe glosa
                 e somente quando não houver registro da data de notificação --}}
            ...
            {{-- Botão Retirada de Ofício de glosa - visível apenas para admin ou equipe glosa
                 e somente quando houver data de notificação, mas não houver data de retirada --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'glosa') && 
                $pacote->localizacao_atual == 'glosa' && 
                $pacote->data_notificacao_glosa != null &&
            ...
            {{-- Botão Recurso Não Recebido - visível apenas para admin ou equipe glosa
                 e somente quando o estado da glosa FOR "Aguardando Recurso de Glosa" --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'glosa') && 
                $pacote->estado_glosa == 'Aguardando Recurso de Glosa')
            ...
            {{-- Botão Análise de Recurso - visível apenas para admin ou equipe glosa
                 e somente quando o estado da glosa FOR "Recurso recebido" --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'glosa') && 
            ...
            {{-- Botão Implantar Pagamento - visível apenas para admin ou equipe sire
                 e somente quando o pacote estiver na localização SIRE ou GLOSA com valor pendente --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'sire') && 
                ...
                <a href="#" class="btn btn-success" data-toggle="modal" data-target="#modalInformarPagamento" 
                   data-id="{{ $pacote->id }}" data-valor-pendente="{{ $pacote->valor_pendente }}">
                    <i class="fas fa-money-bill-wave"></i> Implantar Pagamento
                </a>
            @endif

            {{-- Botão Aguardando Limite de Crédito - visível apenas para admin ou equipe sire
                 e somente quando o pacote estiver na localização SIRE ou GLOSA com valor pendente --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'sire') && 
                ($pacote->localizacao_atual == 'sire' || $pacote->localizacao_atual == 'glosa') && 
                $pacote->valor_pendente > 0)
            ...
            {{-- Botão Arquivar - visível apenas para admin ou equipe arquivo
                 e somente quando o pacote estiver na localização "arquivo" --}}
            ...
            {{-- Botão Editar Localização Física - visível apenas para admin ou equipe arquivo
                 e somente quando o pacote estiver na localização "arquivo" ou "arquivado" --}}
            @if((Auth::user()->role == 'admin' || Auth::user()->role == 'arquivo') && 
    ...
    <div class="modal fade" id="modalAguardandoLimite" tabindex="-1" role="dialog" aria-labelledby="modalAguardandoLimiteLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="formAguardandoLimite" action="{{ route('pacotes.aguardar-limite', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <p>Confirma que o pacote #{{ $pacote->id }} está aguardando limite de crédito?</p>
                        <div class="form-group">
    ...
    <div class="modal fade" id="modalNotificarGlosa" tabindex="-1" role="dialog" aria-labelledby="modalNotificarGlosaLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="formNotificarGlosa" action="{{ route('pacotes.notificar-glosa', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group date" id="data_limite_div" data-target-input="nearest">
                                <input type="text" class="form-control datetimepicker-input" id="data_limite_retirada" 
                                       ...
                                       value="{{ date('d/m/Y', strtotime('+30 days')) }}" required>
    ...
    <div class="modal fade" id="modalRetiradaOficioGlosa" tabindex="-1" role="dialog" aria-labelledby="modalRetiradaOficioGlosaLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="formRetiradaOficioGlosa" action="{{ route('pacotes.retirada-oficio-glosa', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <p>Informe a data em que a OCS/PSA <strong>{{ optional($pacote->ocsPsa)->nome }}</strong> retirou o ofício de glosa:</p>
    ...
    <div class="modal fade" id="modalRecebimentoRecurso" tabindex="-1" role="dialog" aria-labelledby="modalRecebimentoRecursoLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="formRecebimentoRecurso" action="{{ route('pacotes.recebimento-recurso', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <p>Informe a data em que o recurso de glosa foi recebido da OCS/PSA <strong>{{ optional($pacote->ocsPsa)->nome }}</strong>:</p>
    ...
    <div class="modal fade" id="modalRecursoNaoRecebido" tabindex="-1" role="dialog" aria-labelledby="modalRecursoNaoRecebidoLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="formRecursoNaoRecebido" action="{{ route('pacotes.recurso-nao-recebido', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
    ...
    <div class="modal fade" id="modalAnaliseRecurso" tabindex="-1" role="dialog" aria-labelledby="modalAnaliseRecursoLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="formAnaliseRecurso" action="{{ route('pacotes.analise-recurso', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <p>Realize a análise do recurso de glosa apresentado pela OCS/PSA <strong>{{ optional($pacote->ocsPsa)->nome }}</strong>.</p>
                        ...
                        <div id="campos-deferido" style="display: none;">
                            ...
                            <div class="form-group">
                                ...
                                <small class="form-text text-muted">
                                    Valor máximo: R$ {{ number_format($pacote->valor_glosa, 2, ',', '.') }}
                                </small>
                            </div>
    ...
    <div class="modal fade" id="modalArquivar" tabindex="-1" role="dialog" aria-labelledby="modalArquivarLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="formArquivar" action="{{ route('pacotes.arquivar', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="localizacao_fisica" name="localizacao_fisica" 
                                   value="{{ $pacote->localizacao_fisica ?? '' }}" required
    ...
    <div class="modal fade" id="modalEditarLocalizacaoFisica" tabindex="-1" role="dialog" aria-labelledby="modalEditarLocalizacaoFisicaLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="formEditarLocalizacaoFisica" action="{{ route('pacotes.atualizar-localizacao-fisica', $pacote->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btn-mover-pacote').click(function(e) {
                ...
                if (localizacao === 'sire') {
                    var valorGlosa = {{ $pacote->valor_glosa ?? 0 }};
                    var valorPendente = {{ $pacote->valor_pendente ?? 0 }};
                    var valorRecursoGlosa = {{ $pacote->valor_recurso_glosa ?? 0 }};
                    var localizacaoAnterior = "{{ $pacote->localizacao_anterior }}";
            ...
            function configurarModal(pacoteId, localizacao, destino = '') {
                ...
                $('#formMoverPacote').attr('action', '{{ route("pacotes.mover", $pacote->id) }}');
            ...
            $('#modalInformarPagamento').on('show.bs.modal', function(e) {
                ...
                $('#formInformarPagamento').attr('action', '{{ url("pacotes") }}/' + pacoteId + '/pagamento');
            ...
            $('#modalAguardandoLimite').on('show.bs.modal', function(e) {
                ...
                $('#formAguardandoLimite').attr('action', '{{ url("pacotes") }}/' + pacoteId + '/aguardar-limite');
            ...
            $('#data_retirada_div').datetimepicker({
                format: 'DD/MM/YYYY',
                locale: 'pt-br',
                minDate: moment("{{ $pacote->data_notificacao_glosa ? $pacote->data_notificacao_glosa->format('Y-m-d') : null }}"), // Data mínima = data notificação
            ...
            $('#modalRetiradaOficioGlosa').on('shown.bs.modal', function() {
                ...
                var dataNotificacao = moment("{{ $pacote->data_notificacao_glosa ? $pacote->data_notificacao_glosa->format('Y-m-d') : null }}");
